...
# Usage: ./scripts/diag-closer.sh <org_id> [prospect_id]
...
ORG_ID="${1:-}"
PROSPECT_ID="${2:-00000000-0000-0000-0000-000000000001}"

if [[ -z "$ORG_ID" ]]; then
  echo "Usage: $0 <org_id> [prospect_id]" >&2
  exit 1
fi
...
if prospect_id.startswith("00000000"):
    prospect_id = "diag-" + uuid.uuid4().hex[:12]
